...
if __name__ == "__main__":
    ...
    parser.add_argument('-d', '--data_dir', default=path.join(cache, 'datasets', 'EsnliDataSet'))
    parser.add_argument('-s', '--log_dir', default=path.join(cache, 'logs', 'snli_igrida_trained'))
    ...
    log.info(f"lagrange : {args.lagrange}")

    if args.lagrange:
        ckp = path.join(args.log_dir, "regu_study", "lagrange_reg", f"mul={args.reg_mul}", "checkpoints",
                        "best.ckpt")
    ...
    if args.lagrange:
        # lagrange save
        dir = os.path.join(cache, "plots", "e_snli", f"regu_study", "lagrange_reg")
        ...
        # entropy reg
        dir = os.path.join(cache, "plots", "e_snli", f"regu_study", "entropy_regu")
        if not os.path.exists(os.path.join(dir, f"mul={args.reg_mul}")):
            os.mkdir(os.path.join(dir, f"mul={args.reg_mul}"))
        dir = os.path.join(dir, f"mul={args.reg_mul}")
